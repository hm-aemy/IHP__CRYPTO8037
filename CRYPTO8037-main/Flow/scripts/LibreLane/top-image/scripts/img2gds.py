...
def remove_corner_only_connections(pixel_array):
    """
    Removes 2x2 diagonal-only metal contacts that become point-touching
    polygons in GDS and trigger DRC errors.

    For every checkerboard 2x2 pattern, one of the diagonal pixels is cleared.
    We preserve the denser side by removing the pixel with fewer orthogonal
    metal neighbors; ties are broken deterministically by array order.
    """
    height, width = pixel_array.shape

    def orthogonal_neighbor_count(y, x):
        count = 0
        for ny, nx in ((y - 1, x), (y + 1, x), (y, x - 1), (y, x + 1)):
            if 0 <= ny < height and 0 <= nx < width and pixel_array[ny, nx]:
                count += 1
        return count
    ...
    while changed:
        ...
        for y in range(height - 1):
            for x in range(width - 1):
                block = pixel_array[y:y + 2, x:x + 2]

                if block[0, 0] and block[1, 1] and not block[0, 1] and not block[1, 0]:
                    candidates = [(y, x), (y + 1, x + 1)]
                elif block[0, 1] and block[1, 0] and not block[0, 0] and not block[1, 1]:
                    candidates = [(y, x + 1), (y + 1, x)]
                else:
                    continue

                _, clear_y, clear_x = min(
                    (orthogonal_neighbor_count(cy, cx), cy, cx)
                    for cy, cx in candidates
                )
                pixel_array[clear_y, clear_x] = False
                changed = True
                break
...
def convert_to_gds(
    ...
    if apply_slit_rules and pixel_size > 0:
        ...
        if avoid_corner_connections:
            print("[Corner cleanup] removing diagonal-only pixel corner contacts")
            pixel_array = remove_corner_only_connections(pixel_array)
    ...
    elif avoid_corner_connections:
        pixel_array = np.array(new_image_binary, dtype=bool)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        '--allow-corner-connections',
        action='store_true',
        help='keep diagonal-only pixel corner contacts instead of removing them'
    )
    ...
    convert_to_gds(
        ...
        avoid_corner_connections=not args.allow_corner_connections,
    )
